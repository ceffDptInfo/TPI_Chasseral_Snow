...
class Installation
{
    ...
    public function desactive($tab){
        ...
        $query= "DELETE FROM wp_bs_installations_active WHERE (`id_ins`) =:id_ins and `date_ins`=:date_ins LIMIT 1";
        try {
    ...
    public function active($tab){
        $args['id_ins'] = $tab['id_ins'];
        $args['date_ins'] = $tab['date_ins'];
        $query= "INSERT INTO wp_bs_installations_active (`id_ins`, `date_ins`) VALUES
                                                         (:id_ins, :date_ins)";
        try {
            $stmt = $this->pdo->prepare($query);
            $stmt->execute($args);
